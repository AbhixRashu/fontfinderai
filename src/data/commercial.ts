// Curated mapping of commercial typefaces to their recommended free stand-ins.
// The same list the detector uses to pair paid fonts with open-source matches.
export interface CommercialFont {
  name: string;
  foundry: string;
  buyUrl: string;
...
export const COMMERCIAL_FONTS: CommercialFont[] = [
...
export function commercialFor(slug: string): CommercialFont[] {
  return COMMERCIAL_FONTS.filter((f) => f.freeAlternatives.includes(slug));
}
